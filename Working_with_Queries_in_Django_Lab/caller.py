...
def find_books_by_genre_and_language(genre: Book.genre, language: Book.language) -> str:
...
def find_authors_nationalities() -> str:
    ...
    for author in Author.objects.filter(nationality__isnull=False):
        all_authors.append(f"{author} is {author.nationality}")
...
def order_books_by_year() -> str:
    ...
    for book in Book.objects.all().order_by("publication_year", "title"):
        all_books.append(f"{book.publication_year} year: {book.title} by {book.author}")

    return '\n'.join(all_books)


def delete_review_by_id(id: int) -> str:
    deleted_review = Review.objects.get(id=id)
    deleted_review.delete()
    return f"Review by {deleted_review.reviewer_name} was deleted"


def filter_authors_by_nationalities(nationality: Author.nationality) -> str:
    authors = []
    for author in Author.objects.filter(nationality=nationality).order_by('first_name', 'last_name'):
        if author.biography:
            authors.append(f"{author.biography}")
        else:
...
def filter_authors_by_birth_year(year1: int, year2: int) -> str:
    authors_data = []
    for a in Author.objects.filter(birth_date__year__range=(year1, year2)).order_by('-birth_date'):
        authors_data.append(f"{a.birth_date}: {a.first_name} {a.last_name}")

    return '\n'.join(authors_data)


def change_reviewer_name(reviewer_name: Review.reviewer_name, new_name: str) -> QuerySet:
