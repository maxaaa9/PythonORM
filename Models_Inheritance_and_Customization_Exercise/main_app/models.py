...
class StudentIDField(models.PositiveIntegerField):
    def to_python(self, value) -> int or 'ValueError':
        try:
            return int(value)
        except ValueError:
            raise ValueError("Invalid input for student ID")

    def get_prep_value(self, value) -> int or ValidationError:
...
class MaskedCreditCardField(models.CharField):
    ...
    def get_prep_value(self, value):
        final_value = self.to_python(value)
        return f"****-****-****-{str(final_value)[-4:]}"
...
class SpecialReservation(BaseReservation):
    ...
    def extend_reservation(self, days: int) -> ValidationError or str:
